...
CREATE TABLE projects (
  ...
  notion_target_id  TEXT,                                -- 이 프로젝트의 회의록을 보낼 Notion 테이블/페이지
  notion_target_type TEXT,                               -- 'database' | 'page'
  created_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
CREATE TABLE meetings (
  ...
  notion_synced_at TIMESTAMPTZ,                  -- Notion 전송 완료 시각 (일괄 전송 중복 방지)
  notion_page_id   TEXT,                         -- 전송된 Notion 페이지 — 수정 시 교체(업데이트)용
  notion_target_sent TEXT,                       -- 마지막으로 전송한 Notion 대상(정규화 ID) — 대상 변경 감지용
  dooray_target_sent TEXT,                       -- 〃 Dooray 프로젝트 ID
  dooray_synced_at TIMESTAMPTZ                   -- Dooray 전송 완료 시각
);
