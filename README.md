...
> 전체 기능 목록은 **[FEATURES.md](FEATURES.md)** 참고.
...
> 화면 상단 배너의 **[🔧 마이그레이션 실행 (관리자)]** 버튼을 누르세요 — 관리자 로그인 상태에서 한 번에 해결됩니다.
> (수동으로 하려면 **`migrate.sql`** 전체를 Neon SQL Editor에서 실행 — 어떤 버전의 DB든 최신 스키마로 맞추며, 여러 번 실행해도 안전합니다.)
...
- 오디오 STT: 브라우저에서 처리(서버 미경유). **긴 파일(약 15분 초과)은 10분 조각으로 분할**(Web Audio로 16kHz 모노 WAV 재인코딩) → 조각마다 업로드·전사해 결과가 점진적으로 도착. 짧은 파일은 원본 그대로 통짜 전사(품질 손실 없음), 디코딩 실패 시에도 통짜 폴백. aac/m4a/mp3/wav 지원, 최대 500MB (긴 파일도 분할로 처리 — 조각은 항상 ~19MB)
