...
| `APP_PASSWORD` | 앱 로그인 비밀번호 (미설정 시 모든 API 잠김) | ✅ |
...
- **로그인**: 공유 비밀번호(`APP_PASSWORD`) 방식. 서버가 HMAC 서명한 30일 토큰을 발급하고, 모든 데이터 API(`/api/meetings*`)는 토큰 필수(401). 비밀번호 비교·토큰 검증은 상수시간 비교 사용.
...
- **응답 헤더**: `nosniff`, `X-Frame-Options: DENY`(클릭재킹 방지), `Referrer-Policy`, `Permissions-Policy` (vercel.json).
