...
- 오디오 STT: 브라우저에서 처리(서버 미경유). **긴 파일(약 15분 초과)은 10분 조각으로 분할**(Web Audio로 16kHz 모노 WAV 재인코딩) → 조각마다 업로드·전사해 결과가 점진적으로 도착. 짧은 파일은 원본 그대로 통짜 전사(품질 손실 없음), 디코딩 실패 시에도 통짜 폴백. aac/m4a/mp3/wav 지원, 최대 100MB
